...
const Tasks = () => {
  ...
  useEffect(() => {
    if (fetchTask) getAllTasks();
  }, [fetchTask]);

  useEffect(() => {
    if (status && tasks.length) {
      setFilteredTasks(tasks.filter((task) => task.stage === status));
    } else if (tasks.length && !status) {
      setFilteredTasks(tasks);
    }
    console.log("with stage", filteredTasks);
  }, [tasks, status]);
